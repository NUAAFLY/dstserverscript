...
Menu(){
  ...
  do
    ...
    case "${cmd}" in
      ...
      if [ $fclose -eq 1 ]
      then
        tmux ls | cut -d ':' -f1 | while read line
        do
      ...
      while read line
      do
        if [ $( echo $line | grep -c "=> not found" ) -gt 0 ]
        then
            bugstr=$bugstr$line
        fi
...
branching default environment 岔路地形 never 无 least 最少 default 默认 most 最多 random 随机
...
loop default environment 环状地形 never 无 default 默认 always 总是
...
Install_Dependency(){
    ...
    then
      sudo dpkg --add-architecture i386
        sudo apt update
        sudo apt install -y lib32gcc1 libstdc++6 libstdc++6:i386 libcurl4-gnutls-dev:i386 tmux wget lua5.2 openssl libssl-dev curl
    else
      sudo apt update
      sudo apt install -y libstdc++6 libcurl4-gnutls-dev tmux wget lua5.2 openssl libssl-dev curl
    fi
    else
    if [[ "${bit}" = "x86_64" ]]
    then
      sudo yum install -y tmux glibc.i686 libstdc++ libstdc++.i686 libcurl.i686 wget lua5.2 openssl openssl-devel curl
    else
      sudo yum install -y wget tmux libstdc++ libcurl lua5.2 openssl openssl-devel curl
    fi
     fi
}
# Install steamcmd
...
checkmodupdate(){
    ...
    then
    	...
    	tip "MOD(${new_mod_name})已是最新版本: 【${cur_mod_ver}】。"
    	result="false"
    fi
}
Getinputlist(){
